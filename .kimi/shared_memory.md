...
| `app.utils.wecom_crypto` | 企微回调加解密（WXBizMsgCrypt 标准实现） | 签名/解密逻辑分叉 |
| `app.services.agent_tools` | AI Agent 可调用工具的定义 + 执行器 | 工具不一致、各端重复实现 |
...
```
POST /api/v1/wecom/callback（加密 JSON）
  ↓
routers.wecom
  ├── wecom_crypto.decrypt_message() —— 验签 + AES-256-CBC 解密
  ├── WecomBotService.handle_text()    —— 意图路由
  │   ├── 关键词 "库存 / 能否上线"   → 直接调 agent_tools 执行器（本地快照）
  │   └── 其他                        → AgentService（LLM + 工具调用循环）
  └── wecom_crypto.build_encrypted_reply() —— 加密回复
```

**关键差异 vs 其他 router**：
- 回调路由**不走 JWT 认证**（企微回调自带签名验签）
- 回复通过**本次 HTTP 响应体**返回加密包（非异步推），外层 `{encrypt, msgsignature, timestamp, nonce}`

**已知风险**（2026-06-24）：Webhook 短连接模式的"回复信封内层格式"（`aibot_respond_msg`）尚未企微实测验证，已隔离在 `routers/wecom.py` 的 `_build_reply_envelope` + `_extract_text`。联调时若不收回复，只改这两处即可。

### 3.6 云霄库存查询链路（Agent 工具路径）

```
AI Agent / WecomBot 快捷命令
  ↓
agent_tools.exec_query_inventory(zone, instance_family, instance_type)
  ↓
YunxiaoService._get_cached_inventory()  ← 仅读 DB 快照，不触发浏览器
  ↓
YunxiaoInventorySnapshot（7 天有效期）
```

**注意**：`_get_cached_inventory` 是 staticmethod，可被 Agent 工具直接调用而不触发 `YunxiaoClient` 的云端 fallback。Agent 工具调用时须始终传 `session` 参数以走本地路径。
...
| `app.services.wecom_bot_service` | 直接调用 `agent_tools` 执行器（`exec_query_inventory` / `exec_check_online_capacity`），绕过了标准的 router→service 分层 | **有意设计**：关键词快捷命令需要低延迟直接回复，不走 Agent 循环。执行器本身是纯函数，不引入新依赖。如需扩展到"service 装个壳子调执行器"，可后续重构，但现阶段 YAGNI |
...
*Last updated: 2026-06-24 — 新增 §3.5 企微智能机器人回调链路、§3.6 云霄库存查询 Agent 工具路径；§2 新增 wecom_crypto 和 agent_tools 为单一来源；§9 新增 wecom_bot_service 有意架构偏离说明*
